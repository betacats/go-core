...
// ParseJsonBodyRelaxed parses the post request which contains json in body.
func ParseJsonBodyRelaxed(r *http.Request, v interface{}) error {
	if withJsonBody(r) {
		reader := io.LimitReader(r.Body, maxBodyLen)
		dec := json.NewDecoder(reader)
		dec.UseNumber() // number -> json.Number，便于后续转 string/int64
		return dec.Decode(v)
	}
	return mapping.UnmarshalJsonMap(nil, v)
}
